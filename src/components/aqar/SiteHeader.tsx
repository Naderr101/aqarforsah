...
export function SiteHeader(){
 ...
 return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur">
  <div className="mx-auto grid h-16 max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 lg:flex lg:h-[72px] lg:px-8">
   <Brand/><nav className="mr-auto hidden items-center gap-7 lg:flex">{links.map((l,i)=><Link key={`${l.to}-${i}`} to={l.to} className="border-b-2 border-transparent py-6 text-sm font-bold text-foreground/80 transition-colors hover:border-primary hover:text-primary" activeProps={{className:"border-primary text-primary"}}>{l.label}</Link>)}</nav>
   <div className="hidden items-center gap-2 lg:flex"><Button variant="ghost" size="icon" aria-label="المفضلة" asChild><Link to="/favorites"><Heart/></Link></Button><Button variant="ghost" size="sm"><Globe2/> EN</Button>{signedIn?<><Button variant="outline" asChild><Link to="/dashboard/exit-requests">طلبات الخروج</Link></Button><Button variant="ghost" onClick={signOut}>خروج</Button></>:<><Button variant="outline" asChild><Link to="/login" search={{redirect:undefined}}>تسجيل الدخول</Link></Button><Button asChild><Link to="/register" search={{redirect:undefined}}>إنشاء حساب</Link></Button></>}</div>
  ...
  {open&&<nav className="border-t bg-background px-4 py-4 lg:hidden">{links.map((l,i)=><Link key={`${l.to}-${i}`} to={l.to} onClick={()=>setOpen(false)} className="block border-b py-3 font-bold">{l.label}</Link>)}<div className="mt-4 grid grid-cols-2 gap-2">{signedIn?<><Button variant="outline" asChild><Link to="/dashboard/exit-requests" onClick={()=>setOpen(false)}>طلبات الخروج</Link></Button><Button variant="ghost" onClick={signOut}>خروج</Button></>:<><Button variant="outline" asChild><Link to="/login" search={{redirect:undefined}}>دخول</Link></Button><Button asChild><Link to="/register" search={{redirect:undefined}}>إنشاء حساب</Link></Button></>}</div></nav>}
 </header>
}
