...
export function SiteHeader(){
 ...
 return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur">
  <div className="mx-auto grid h-16 max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 lg:flex lg:h-[72px] lg:px-8">
   ...
   <div className="hidden items-center gap-2 lg:flex"><Button variant="ghost" size="icon" aria-label="المفضلة" asChild><Link to="/favorites"><Heart/></Link></Button><Button variant="ghost" size="sm"><Globe2/> EN</Button><Button variant="outline" asChild><Link to="/login">تسجيل الدخول</Link></Button><Button asChild><Link to="/register">إنشاء حساب</Link></Button></div>
  ...
  {open&&<nav className="border-t bg-background px-4 py-4 lg:hidden">{links.map((l,i)=><Link key={`${l.to}-${i}`} to={l.to} onClick={()=>setOpen(false)} className="block border-b py-3 font-bold">{l.label}</Link>)}<div className="mt-4 grid grid-cols-2 gap-2"><Button variant="outline" asChild><Link to="/login">دخول</Link></Button><Button asChild><Link to="/register">إنشاء حساب</Link></Button></div></nav>}
 </header>
}
