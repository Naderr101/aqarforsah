...
function StaffLink({onClick}:{onClick?:()=>void}){const q=useMyAccess();if(!q.data?.staff)return null;return <Button variant="ghost" asChild><Link to="/staff/verification" onClick={onClick}>لوحة المراجعة</Link></Button>}
...
export function SiteHeader(){
 ...
 return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur">
  ...
  {open&&<nav className="border-t bg-background px-4 py-4 lg:hidden">{links.map((l,i)=><Link key={`${l.to}-${i}`} to={l.to} onClick={()=>setOpen(false)} className="block border-b py-3 font-bold">{l.label}</Link>)}<div className="mt-4 grid grid-cols-2 gap-2">{signedIn?<><StaffLink onClick={()=>setOpen(false)}/><Button variant="outline" asChild><Link to="/dashboard/exit-requests" onClick={()=>setOpen(false)}>طلبات الخروج</Link></Button><Button variant="ghost" onClick={signOut}>خروج</Button></>:<><Button variant="outline" asChild><Link to="/login" search={{redirect:undefined}}>دخول</Link></Button><Button asChild><Link to="/register" search={{redirect:undefined}}>إنشاء حساب</Link></Button></>}</div></nav>}
 </header>
}
